...
void TbxProc::charInputEvent(wchar_t c)
{
	if (first_hl_num != second_hl_num)
	{
		backspaceEvent(); //îïòèìèçèðîâàòü, ÷òîáû ëèøíèé ðàç íå îáíîâëÿòü
	}
	...
	if (first_hl_box.getLeft() + 1.0 > view_pos + view_width - indent) //çàìåíèòü íà second_hl_box?
	{
...
{
	txt_pos = loadTxtBounds(view_pos, view_width, txt_pos, txt_width); //òåêñò äîëæåí íà÷èíàòñÿ íà íåêòîðîì óäàëåíèè îò íà÷àëà
...
void TextBox::update(WMInterfaceData& wm_dat, RenderWindow& window) //Áàã - íàâåñòè íà ïîëå óæå çàæàòóþ ìûøêó
{
	//ÑÏÈÑÎÊ ÄÅË:
	//ÐÀÑÑÒÎßÍÈÅ ÎÒ ÊÐÀÅÂ - 0.5
	//ñÎÊÐÎÑÒÜ ÏÅÐÌÅÙÅÍÈß ÇÀÂÈÑÈÌÀ ÎÒ ÁËÈÇÎÑÒÈ ÊÓÐÑÎÐÀ Ê ÊÐÀÞ
	int i;
	MouseData md;
